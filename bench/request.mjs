...
// pass after it.
const budget = perRoute * routes.length * 3 + 128;
...
if (asJson) {
  console.log(JSON.stringify({ requests: perRoute, atOnce, routes: measured, throughput }, null, 2));
} else {
  console.log(`${perRoute} requests a route, each on its own connection\n`);
  console.log("route          median    slowest 10%   alone/s   " + String(atOnce) + " at once/s");
