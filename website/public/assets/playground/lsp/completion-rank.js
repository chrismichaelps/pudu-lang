...
//
// Names that start with the typed text come first, then names that contain its
// letters in order. Within each group the server's order is kept, which puts
// the nearest names first.
...
export function rankCompletions(items, typed) {
  ...
  for (const item of items) {
    ...
    if (label.startsWith(lower)) {
      prefixed.push({ item, marks: typed ? [[0, typed.length]] : [] });
    } else if (lower) {
      const marks = subsequenceMarks(label, lower);
      if (marks) scattered.push({ item, marks });
    }
  }
  return prefixed.concat(scattered).slice(0, MAX_COMPLETIONS_SHOWN);
}
