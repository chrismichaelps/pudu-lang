...
- A line-leading binary operator with no prefix form is consumed by [[Parser Expression]] before block parsing can see it. `-`, `&`, and `*` remain ordinary statement starts, except that an expression which already established a line-leading binary chain reports `E1055` before returning them to this module; block recovery then preserves the following unary statement rather than swallowing it.
