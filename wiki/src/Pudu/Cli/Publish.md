...
- **Q:** What does a release do with a module outside the package's root? **A:** Names each one on stderr and publishes anyway. _Rationale:_ [[architecture/PACKAGES]] allows such a module and requires the report at publish time; a program with a module of the same name shadows it, which the publisher can only fix before releasing. `Main.pudu` beside the root is an executable entry nothing imports, so it is not reported. _Rejected:_ refusing the release, a change of the architecture's semantics left to Language Architect review (#304); a check in `pudu check`, which runs on every edit and cannot know the tree is about to be published.
