...
- `charAtFast`, `length`, and `slice` read long texts (256 bytes or more) through a two-entry cursor keyed by the text's buffer, offset, and byte length. An entry remembers the last scalar index, its byte offset, and the counted scalar length, so a scan by position walks only the distance it moved, backwards when the remembered position is nearer than the start. Each entry is an immutable record and the pair is replaced atomically, so concurrent readers see a pair correct for its own texts. Measured at -O2, a `charAt` scan of 640,000 ASCII characters fell from 11.0 s to 0.94 s and 160,000 mixed-width characters from 0.97 s to 0.27 s.
