...
machine-readable conformance report containing configuration, artifact locations, observed counts,
...
portable public packages while making missing speaker output, codecs, native input, display-link
...
- No claim that writing a WAV means speaker playback worked.
...
- **Q:** Use `afplay` to make the WAV audible? **A:** No. _Rationale:_ that tests a child process,
  not Pudu's audio device contract, and is macOS-only. _Rejected:_ shelling out as presentation.
