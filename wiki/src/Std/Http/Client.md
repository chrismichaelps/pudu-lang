...
ends can read.
## Interface
What a request may cost: how long the whole exchange may take, how many redirects it may follow, how
large an answer it will read, and which addresses it may reach. Sending a request to a URL and
receiving the response. Fetching a URL without building a request first. The refusals, and what each
says.
...
sender did not send. The transport stops reading once the body allowance plus a fixed framing
allowance has arrived; checking only after an unbounded `receiveAll` would report the refusal after
...
timeout would let a five-redirect request take six times what the caller stated. Zero means the
request may not wait; expiration is `TimedOut`, distinct from an unreachable address because retry
policy differs. Negative or directly constructed negative limits normalize to zero, so the HTTP
client has no unbounded mode. A timed stream operation invalidates its connection, and TLS cleanup
spends only the remaining budget before closing the socket unconditionally.
...
The deterministic fixture owns protocol decisions and runs against loopback. A separate public-web
fixture sends both secured and plain requests to the IANA-reserved example domain, checks the status
and a stable page marker, then fetches one stable JSONPlaceholder resource and checks its documented
fields. It runs only from the scheduled or manually dispatched integration gate. This proves that
DNS, the machine trust store, TLS, real sockets, request rendering, transfer decoding, and response
parsing interoperate with servers outside this repository. It is not in the ordinary suite: an
internet outage or third-party outage is not a compiler regression, and making either one block
every pull request would teach people to ignore a red gate.

The external gate complements rather than replaces loopback. Public servers cannot safely be made
to emit every malformed framing, redirect, credential, and size-limit case this module must refuse;
...
- **Q:** Read the whole response and check the limit afterwards? **A:** No. _Rationale:_ that reports
  `TooLarge` only after the peer has already exhausted the memory the bound exists to protect.
  _Rejected:_ an unbounded transport read followed by a bounded parse.
...
- **Q:** Give connect, send, receive, and every redirect their own timeout? **A:** No. _Rationale:_
