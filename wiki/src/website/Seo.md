...
The home page carries a JSON-LD `@graph` of a `WebSite` (named `Pudu`, with the alternate names
search results may show and a `SearchAction` into `/search?q=`) and the `Organization` behind it, with
the 512-pixel mark as its logo and the repository as `sameAs`. That is what lets a result show the
site as "Pudu" and with its logo rather than its domain.
...
- **Q:** Why did search results show a globe for the site? **A:** Its only icon was 64 pixels, and
  Google shows a favicon only at a multiple of 48 pixels, ideally with `/favicon.ico` at the root.
  _Resolution:_ the icon set in [[website View Layout]].
