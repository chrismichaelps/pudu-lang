...
Enhances the native install disclosure with a version picker that rewrites the displayed command and copy buttons for commands and import lines. It binds the Source tab's file filter and [[Package source navigation]], and loads the documentation's code-copy script so a README's code blocks copy as the documentation's do ([[website View Docs]]). It also observes a bounded list's next-page link, fetches the next HTML page near the scroll edge, and appends only its rows. The link remains usable on failure or without script.
