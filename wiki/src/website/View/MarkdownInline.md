...
Renders one line of Markdown prose: `` `code` ``, `**strong**`, `*emphasis*`, `[label](target)`
links, and `![alt](source)` images, with everything between them as text. A plain run is taken up to the next marker in one step.
...
website suite checks it. `renderWith` takes the [[website View MarkdownHtml]] `Targets` a package
file resolves relative links and images against; `render` refuses them. An image is made only when
its source resolves, and is otherwise shown as written.
