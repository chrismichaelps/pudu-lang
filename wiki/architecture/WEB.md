...
| Authentication? | **Ready** | [[Std App Password]] carries the work factor it was made with, so raising it locks nobody out. [[Std App Session]] issues a new name on every change of privilege, which is the fixation defence made unforgettable rather than documented. [[Std App Totp]] provides RFC 6238 time-based one-time passwords for multi-factor authentication with Base32 codecs, clock skew tolerance, and replay prevention. |
