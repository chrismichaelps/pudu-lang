...
| Critical | Mutation through references | The checker admits `*reference = value` and assignment to a field reached through `&mut`, but the evaluator treats `&`, `&mut`, and `*` as the value itself and its assignment accepts only a plain name, so each such assignment stops the program with `E7001 assignment target is not a place` (measured 2026-09-15 with a dereference, a `mut` field through `&mut`, and a local `&mut`). No fixture exercised it: `UsesVarint` declared a helper that assigned through `*held` and never called it. Assignment to a field, an element, or through a reference is now refused at check time with `E3077`, so no program checks and then stops on it; the documentation shows building a new value or returning the changed one. | The check-time refusal ships with the first release. Implementing places for references and `mut` fields in the evaluator remains before those forms are admitted. |
