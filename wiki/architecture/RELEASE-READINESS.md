...
| Critical | Mutation through references | The checker admits `*reference = value` and assignment to a field reached through `&mut`, but the evaluator treats `&`, `&mut`, and `*` as the value itself and its assignment accepts only a plain name, so each such assignment stops the program with `E7001 assignment target is not a place` (measured 2026-09-15 with a dereference, a `mut` field through `&mut`, and a local `&mut`). No fixture exercises it: `UsesVarint` declares a helper that assigns through `*held` and never calls it. The documentation states the limitation and shows returning the changed value instead. | Either implement places for references in the evaluator or refuse such assignments at check time with a diagnostic; a program must not check and then fail on a documented form. |
