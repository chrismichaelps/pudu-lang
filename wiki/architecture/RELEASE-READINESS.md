...
| Critical | Large-input bounds evidence | Streaming APIs exist across IO, protocols, and formats, but the standard-library completion queue still lacks residency-bounded fixtures independent of input size. | Must complete for production-readiness claims on those readers. |
...
| High | Filesystem safety surface | Streaming files and portable lexical paths ship. Permissions, race-resistant temporary file/directory creation, symlink-aware containment, and an explicit atomic-replacement API remain absent. | Complete as focused APIs; do not overload lexical `Std.Path` with filesystem effects. |
