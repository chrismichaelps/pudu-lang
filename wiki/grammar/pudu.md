...
- A block yields its final unterminated expression statement. Without one, it yields `Never` when its final statement is a direct `return`, `break`, or `continue`, because that path cannot reach the block boundary; every other resultless block yields `()`. A nested control form that may fall through does not make its containing block `Never` merely because one of its own paths transfers control.
...
- **Text reads itself as a number.** `toInt()`, `toFloat()`, and `toDecimal()` are built-in `Str` methods answering `Option[Int]`, `Option[Float]`, and `Option[Decimal]`. Each reads the whole text — an optional sign, then the number — with no surrounding space and no digit separators, so the same text is accepted or refused the same way everywhere. `toInt` requires ASCII digits fitting `Int`; `toFloat` requires digits on at least one side of an optional point and an optional exponent, rounds correctly, and refuses `inf`, `nan`, and values past binary64; `toDecimal` keeps the digits and scale written.
- **A value has methods from exactly two places**: the closed sets the compiler wires into `Array`, `Str`, `Map`, `Set`, and `Char` — together with `toText()`, which every value answers — and the `impl` blocks a program writes. Everything else is a module function called on the value as an argument — `Option.unwrapOr(value, fallback)`, not `value.unwrapOr(fallback)` — because `Option` is an ordinary sum type and nothing has implemented anything for it. A program that wants method syntax for its own type writes an `impl`, which is the same mechanism the standard library would use.
- **Every value answers `toText()`** with the text `display` writes for it — the text interpolation places — typed `fn() -> Str`. A field or method named `toText` that the value's type declares, or a trait bound supplies, is found first and wins — also where the value is reached through an unbounded type parameter — so a type renders its own way by implementing one. `Bytes` keeps its own `toText()`, which answers `Option[Str]` because not every byte sequence is text. The method is a value like any other method: `let render = count.toText` binds the receiver. Text had no method of this kind on any type but `Char`, so a message built from a number, a flag, and a record mixed three spellings; one name that every value answers is what lets a writer turn a value into text without first knowing its kind.
...
- A function literal is written `fn(params) => expression` or `fn(params) -> Type { block }`, reusing the same `fn` the function type uses. It is also written `|params| expression`, with `||expression` for one that takes nothing and `|params| -> Type { block }` where the result is worth stating. The two spellings build the same value and nothing after the parser can tell which was written; `fn` says what a function *is*, which a declaration needs, while an argument to `map` does not, and three tokens of ceremony around a one-token body is the difference between a program that reads as what it does and one that reads as how it is spelled. The bars cannot be mistaken for the operator or the variant separator they share a spelling with, because a bar in operand position is never either of those. `async` precedes both spellings. A line beginning with a literal's bar starts a statement rather than continuing the line above, which is what lets a literal be a block's result — the place a literal most often goes in a language whose blocks are expressions. It captures the bindings in scope where it is written, takes no default arguments, and is not generalized: its type is fixed at the point it appears. What it captures is a **copy**, so an assignment to a captured name would write the copy and leave the original as it was — the program would state an intent nothing carries out, and the value read afterwards would simply be the old one somewhere else. That assignment is `E3076`, written where the assignment is: return the value instead, or carry it in what the closure answers. A name the closure itself declared is its own and may be assigned freely, and reaching through a captured value to a field or element is a question about that value rather than about the capture.
- **A range is a value**, written `a..b`, `a..=b`, `a..`, `..b`, `..=b`, or `..`. It holds its two ends and a rule for reading them rather than the values between them, so `0..1_000_000` is three fields and not a million: it may be bound to a name, passed to a function, stored, compared, and rendered as it was written. Its ends are whole numbers, and its type is `Range[Int]`. `for x in range` walks it one value at a time without building it, so the memory a loop over a range costs does not depend on how far it counts. A range missing either end has no finite extent, so it cannot be measured, walked, or handed back as values, and a method that needs both ends is refused with `E7004` rather than returning a made-up length or running out of memory; `contains` still answers, since a missing end excludes nothing. A range cannot be chained — `1..2..3` names no value and is `E1062` — and `..=` with no end is `E1063`, since an inclusive end that is not written includes nothing in particular. Built-in methods: `length()`, `isEmpty()`, `isBounded()`, `isInclusive()`, `contains(x)`, `start()`, `end()`, `toArray()`, `reverse()`, `step(n)`, `sum()`, `map(f)`, `filter(f)`, `reduce(f, init)`. `length` and `contains` are arithmetic and answer in the same time however far the range reaches; `length` and `sum` still report `E7005` if their exact answer does not fit `Int`. Everything that skips or reverses answers with an `Array`, because a range counts upward by one and a value that does not cannot answer `contains` or `length` the way a range does.
- **Indexing by a range is a slice**: `items[2..5]`, `items[2..]`, `items[..5]`, `items[..]`, `items[1..=3]`. A stretch of an array is an array, of text is text, of bytes is bytes; a tuple is refused with `E3006`, because its members may differ and the type of a stretch of one depends on which stretch. An absent end is answered by the value being sliced rather than by the writer measuring it first. A slice must lie within the value and be in order: reaching past the end is `E7004` rather than a quiet clamp, because a clamped slice hands back a different sequence than the one asked for and hides the arithmetic that produced the bounds.
...
- **`let PATTERN = EXPRESSION` takes a value apart** where the pattern is one a binding can answer: `let {x, y} = point`, `let (a, b) = pair`, `let [head, ..rest] = items`. `var` binds parts that may be assigned, and an optional annotation after the pattern describes the subject as a whole — the only place a reader can state the type of a value that never receives one name. The names it introduces belong to the block the statement stands in, and the subject is read against the scope as it stands, so `let [x, ..rest] = x` reads the outer `x`. A pattern that tests a tag, a literal, or a range has nowhere to go when it does not apply and is `E1059`: write `let … else`, which does. A sequence's length is the one exception — it is checked where the binding runs and reported there with `E7013`, exactly as reading past the end of the same sequence already is, because requiring a fallback for it would mean writing one at every `let [first, ..rest] = items`.
- **A sequence pattern** is written `[a, b]`, `[first, ..rest]`, `[..front, last]`, or `[first, ..middle, last]`. What precedes the `..` is taken from the front and what follows it from the back; the rest holds everything they did not take, and `..` or `.._` skips it instead of naming it. At most one `..` is admitted, because two would leave no way to say which holds a given element, and a second is `E1050`. It matches an array. A tuple is not a sequence here: its members may differ, and binding them all at the one element type the pattern implies would claim something the tuple does not say — take a tuple apart with `let (a, b) = pair`.
- `let PATTERN = EXPRESSION else BLOCK` binds a refutable pattern for the **remainder of the enclosing block**. The expression evaluates once. The `else` block must not fall through — its type is `Never`, which `return`, `break`, `continue`, and a diverging call all satisfy — and `E3036` says so when it can. That divergence is what earns the binding its scope: every statement after it runs only where the pattern matched. A syntactically irrefutable pattern is `E1057`: use ordinary `let` when matching cannot fail. Which form a `let` opens is settled by the token after it, since a binding names a value: a lowercase identifier is always the ordinary binding, and a constructor path, tuple, record, sequence, literal, or `_` opens a pattern. Whether that pattern form carries a fallback is settled by the `else`, and each form asks the opposite thing of the pattern — with a fallback it must be able to fail, without one it must not test anything a binding cannot recover from. `E1057` therefore reports `let _ = v else { … }` and `let (a, b) = pair else { … }`, which cannot fail, and `E1059` reports `let Some(x) = v` with no fallback, which can. Where `if let` binds inside its own block, `let … else` binds after itself, and neither subsumes the other — the first is for a value used once, the second for a value every following step depends on.
...
- A range is not an operator on two values but a form with two optional ends, so it sits at its band rather than in the binary table: `0..n + 1` runs to `n + 1` and `x in 0..n` compares the range. Whether an end is written is decided by the token that follows, and only a token that could begin an expression is read as one — which is what lets `items[2..]` read the bracket as the range ending. `..` and `..=` have a prefix form, so a line beginning with one starts a statement rather than continuing the line above, exactly as `-`, `&`, and `*` do.
...
- Blocks evaluate statements left-to-right and yield the final unterminated expression. Without one, a final direct control transfer yields `Never`; any fallthrough ending yields `()`.
...
- A range endpoint is evaluated exactly once, where the range is written rather than where it is used, so a range held in a binding and read twice reads its ends once each.
...
- **Places.** A variable, a `mut` field of a place, an element of an array place, and `*r` for an exclusive reference `r` can be assigned; the root must be declared `var` or be reached through a `&mut` parameter, and nothing is written through `&T` (`E3076`–`E3080`). `&mut place` is written only as a call argument, lends the place for that call, and two loans to one call may not overlap (`E3081`, `E3082`). `&mut T` is only the type of a parameter, and no binding, field, result, closure, or type argument holds one (`E3083`, `E3084`). See [[ADR-0022-lending-a-place]].
...
- The types that may cross are stated: `Int8` `Int16` `Int32` `Int64`, `UInt8` `UInt16` `UInt32` `UInt64`, `Float32` `Float64`, `Bool`, `Str`, `Bytes`, `()`, and an opaque handle type declared by the same block. These are the language's own types, so a foreign signature is an ordinary signature and a caller passing the wrong width or handle kind is told so by the ordinary checker. `()` is a result only, since there is no void value to place in an argument. A function may declare at most 32 arguments. Anything else is refused at the declaration, which is the point of stating the list.
- A record whose scalar or `Str` fields cross may cross by value, in the order its declaration writes the fields. It is an ordinary Pudu record, declared beside the foreign block that names it. At most 32 fields, and a record may hold records: validation enumerates the scalar leaves in declaration order, but the native bridge preserves the declared aggregate nesting when it asks the platform for offsets, alignment, and call classification. Flattening the nested shape is not ABI-equivalent. A record reached from inside itself has no finite layout and is refused where it is written, as are a `()` field and an opaque-handle field. A handle stays top-level because that is where its liveness lease and owned-result release are enforced. Where a field sits inside one is the platform's answer, asked for rather than calculated. `Str` is UTF-8 in direct and record positions; invalid returned bytes and nought text addresses are runtime refusals rather than replacement text or unit.
- `Bytes` crosses as the address of the run it names, for the length of the call and no longer. It is lent rather than copied, the library reading storage the value already had, which is what makes handing over a large run cost nothing. Its length travels as an ordinary argument because in C it is one: a run of bytes carries no terminator, so a library is told how far to read the way its other callers tell it. The library may only read: a value that changed underneath the program would falsify every comparison and hash already taken from it. `Bytes` is not admitted as a result, a run the library allocated needing a length and a release that no declaration can yet write, and not as a record field, because a borrowed run is not one scalar or nested aggregate member of a by-value C record.
...
- `owned T by release` is admitted only when `T` is an opaque type declared by the block, and is written on a result or on an output slot. `owned` on a parameter the caller supplies is refused — the value was already theirs and passing it transfers nothing. A release may not declare slots. The release must be declared in that block with exactly one `T` parameter and a `()` result. The runtime refuses a null handle result and refuses an already released or unowned handle before entering foreign code.
- `counted T by release` is the third: the result is another reference to something the library counts, and owes its own release even when an existing reference is the same address. `cairo_reference`, `g_object_ref`, `CFRetain` and `AddRef` all return the pointer they were given, so a boundary keyed on the address would either refuse the second reference or drop one the program still holds. Each reference is a claim of its own, releases once, and is released at teardown if the program did not. A counted result naming no release is `E3075`. Nothing checks that a type is counted by every function that returns it; declaring one owned in one place and counted in another is a mis-declaration this side cannot see.
- `borrowed T` is the other half of a handle result: the library keeps what it returned — a default font, the text of a last error, the surface a context draws to — and nothing on this side releases it. It is claimed by no store, leased by no call, released at no teardown, and refused before native code if it is passed to the release its handle type declared. Handing one to some other foreign function that happens to free it is an ordinary unsafe call, exactly as it is for an owned handle: a function is a release because a `by` clause named it, not because of what it does. A handle result must say which it is; saying neither is `E3066`, because an address does not say, and guessing leaks or frees something still in use. Naming a release for a borrowed result is `E3074`. Whether a borrowed value outlives what owns it is not checked, and is an assertion of the same kind as every other one inside `unsafe(foreign)`. A handle *slot* must still be owned.
